/* =========================================================
   ...
   1. PROXY MODE (recommended for a PUBLIC repo — use this if you want
      one key you provide, shared by everyone, that stays hidden):
      deploy /worker/openrouter-worker.js (free Cloudflare Worker, ~5
      minutes, steps in that file and in README.md) and paste its URL
      into ASSISTANT_PROXY_URL below. Your real key then lives only in
      the worker's environment variables — never in this repo, never
      sent to the browser. This is the ONLY safe way to ship a single
      default key in a public project.

   2. BYO KEY (fallback, used automatically if ASSISTANT_PROXY_URL is
      blank): each visitor pastes their own free OpenRouter key into
      Settings. It is stored only in that browser's localStorage and
      sent only to OpenRouter, directly from the browser.
...
function getStoredKey(){ return localStorage.getItem(OR_KEY_STORAGE) || TEMP_HARDCODED_KEY || ''; }
...
function renderChatWidget(){
  ...
  root.innerHTML = `
    <button id="chatToggleBtn" class="chat-fab" aria-expanded="${chatOpen}" aria-controls="chatPanel">
    ...
    <section id="chatPanel" class="chat-panel" ${chatOpen ? '' : 'hidden'} aria-label="Help Assistant chat">
      <div class="chat-panel-head">
        ...
        <button id="chatCloseBtn" class="chat-close" aria-label="Close chat">×</button>
      </div>
      ${assistantConfigured() ? renderChatBody() : renderChatSetup()}
    </section>
  `;

  $('#chatToggleBtn').onclick = ()=>{ chatOpen = !chatOpen; renderChatWidget(); };
  const closeBtn = document.getElementById('chatCloseBtn');
  if(closeBtn) closeBtn.onclick = ()=>{ chatOpen = false; renderChatWidget(); };
